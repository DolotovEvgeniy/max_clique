...
/*void renumber(const Graph& graph, Vertex p, int NoP, int NoTH, ColorSets &colors) {
    for(int k1 = 0; k1 < NoTH; k1++) {
        if((colors[k1] & graph.getNeighbors(p)).size() == 1) {
            Vertex q = (colors[k1] & graph.getNeighbors(p))[0];
            for(int k2 = k1+1; k2 < NoTH+1; k2++) {
                if((colors[k1] & graph.getNeighbors(p)).size() == 0) {
                    colors[NoP] = colors[NoP]/p;
                    colors[k1] = (colors[k1]/q)|p;
                    colors[k2] = colors[k2]|q;
                }
            }
        }
    }
...
ColorSets recoloring(const Graph & graph,
    ...
    vector<Vertex> Va = candidates;
    printClique(Va);
    graph.sortByDegree(Va);
    printClique(Va);
    ColorSets colors;//={{}};
    map<Vertex, Color> vertex2color;
    int NoTH = maxClique.size()-currClique.size();
    for (int i = 0; i < candidates.size(); i++) {
        cout << "colors: " <<i<< endl;
        for(int j = 0; j < colors.size();j++) {
            printClique(colors[j]);
        }
        cout << "colors end" << endl;
        Vertex p = Va[i];
        int k = 0;
        cout << "start increase" << endl;
        cout << "size: " << colors.size() << endl;
        while(k+1 <= colors.size() && !(colors[k] & graph.getNeighbors(p)).empty()) {
            k++;
            cout << "k: " << k << endl;
        }
        cout << "end increase, k:"<<k << endl;
        if(k+1>colors.size()) {
            colors.push_back({});
        }
        colors[k].push_back(p);
        vertex2color[p] = k;
        cout << "here1" << endl;
        if(k > NoTH - 1 && k +1== colors.size()) {
            cout << "start renumber" << endl;
            renumber(graph, p, k, NoTH-1, colors);
            cout << "end renumber" << endl;
            if(colors.back().empty()) {
    ...
    cout << "sort count" << candidates.size() <<  endl;
    ColorSets newColors;
    cout << "colors count:" << newColors.size() << endl;
    sort(candidates.begin(), candidates.end(),
         [&](const Vertex&v1, const Vertex& v2) {return vertex2color[v1]<vertex2color[v2];});
    cout << "sort end" << candidates.size() <<  endl;
    for(int c: candidates) {
        cout << c << endl;
    }
    for(int i = 0; i < candidates.size(); i++) {
        cout << "i:" << i << endl;
        cout << "vertex2color" << vertex2color[candidates[i]] << endl;
        if(newColors.size() <vertex2color[candidates[i]]+1) {
            cout<<"jjj" << endl;
            newColors.push_back({candidates[i]});
        } else {
            newColors[vertex2color[candidates[i]]].push_back(candidates[i]);
        }
    }
    return newColors;
}*/
...
void findMaxClique(const Graph & graph,
    ...
    int i;
    // cin >> i;
    cout << "print max clique" << endl;
    printClique(maxClique);
    cout << "curr clique" << endl;
    printClique(currClique);
    ...
    while(!candidates.empty()) {
        ...
        if(!newCandidates.empty()) {
            ...
            if(currClique.size() + vColor <= 1+maxClique.size()+theta) {
                ...
                newColors =  /*recoloring(graph, newCandidates);*/ graph.colorVertices(newCandidates);//recoloring(graph, newCandidates);
